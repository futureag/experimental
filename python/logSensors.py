...
from config import device_1, device_2, data_logger_sample_interval, logging_devices, log_data_locally, log_data_via_mqtt
...
def start_sensor_data_logger(mqtt_client):
   ...
   while True:
      ...
      if time_to_sample():
         if log_data_locally == True or log_data_via_mqtt == True:
            for x in logging_devices:
               for y in x['attributes']:
                  ...
                  if log_data_locally:
                     logData(x['name'], "Success", y['name'], y['subject'], "{:+.1f}".format(attribute_val), y['units'], '')
